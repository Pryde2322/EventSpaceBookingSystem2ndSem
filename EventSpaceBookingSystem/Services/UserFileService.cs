...
namespace EventSpaceBookingSystem.Services
{
    public static class UserFileService
    {
        ...
        public static async Task SaveUserAsync(Users user)
        {
            try
            {
                ...
                else
                {
                    ...
                    // 🛎️ Notify user account created/updated
        ...
        public static async Task<List<EventModel>> LoadAllEventSpacesAsync()
        {
            ...
            // Step 1: Load active owners from EventSpaceOwners.txt
            var activeOwnerIds = new List<int>();

            string ownersPath = Path.Combine(JsonDirectory, "EventSpaceOwners.txt");
            ...
            // Step 2: Only load event files from active owners
        ...
        public static async Task SaveEventAsync(EventModel eventModel, string ownerId)
        {
            ...
            var serializedEvents = JsonSerializer.Serialize(events, new JsonSerializerOptions { WriteIndented = true });
            await File.WriteAllTextAsync(filePath, serializedEvents);
        }

        private static async Task<int> SaveToFileAsync(Users user, string filePath)
        {
            var users = await LoadUsersAsync(filePath);
            int userId = users.Count + 1; // Generate a new ID based on the count
            user.Id = userId; // Assign the generated ID to the user

            users.Add(user);

            var serializedUsers = JsonSerializer.Serialize(users, new JsonSerializerOptions { WriteIndented = true });
        ...
        {
            // File name format: EO-[ID].txt
            string fileName = $"EO-{userId}.txt";
            string filePath = Path.Combine(JsonDirectory, fileName);

            if (!File.Exists(filePath))
            {
                var serializedUser = JsonSerializer.Serialize(user, new JsonSerializerOptions { WriteIndented = true });
                await File.WriteAllTextAsync(filePath, serializedUser);
            }
        }
        ...
        {
            string filePath = Path.Combine(GetJsonDirectory, $"S-{username}.txt");
        ...
        public static async Task<int> FindOwnerIdByEventTitleAsync(string headline)
        {
            ...
            foreach (var file in files)
            {
                try
                {
                    ...
                    if (match != null)
                    {
                        // Extract ownerId from filename EO-1.txt
                        string fileName = Path.GetFileNameWithoutExtension(file); // EO-1
        ...
        public static async Task<List<string>> SaveOwnerImagesAsync(int ownerId, ObservableCollection<ImageSource> images)
        {
            ...
            foreach (var image in images)
            {
                ...
                savedPaths.Add(relativePath.Replace("\\", "/")); // Normalize for cross-platform
            }
        ...
        public static async Task<string> SaveUserAvatarAsync(int userId, FileResult file)
        {
            ...
            try
            {
                // Create directory: Json/User Images/[userId]/
                string userFolder = Path.Combine(JsonDirectory, "User Images", userId.ToString());
                Directory.CreateDirectory(userFolder);

                // Generate unique filename (avatar_1.png, avatar_2.png, etc.)
