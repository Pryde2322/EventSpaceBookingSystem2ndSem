...
{
    public partial class EventSpaceOwnerJoinPage : ContentPage
    {
        private readonly EventSpaceOwnerJoinViewModel _viewModel;

        public EventSpaceOwnerJoinPage(string username, string email)
        {
            InitializeComponent();

            _viewModel = new EventSpaceOwnerJoinViewModel(email)
            {
                Username = username
            };
        ...
        {
            await Navigation.PushAsync(new HomePage(_viewModel.Username, _viewModel.FormattedEmail));
        }

        private async void OnExploreClicked(object sender, EventArgs e)
        {
            await Navigation.PushAsync(new VenuePage(_viewModel.Username));
        }

        private async void OnCartClicked(object sender, EventArgs e)
        {
            await Navigation.PushAsync(new CartPage(_viewModel.Username, _viewModel.FormattedEmail));
        }

        private async void OnBecomeOwnerClicked(object sender, EventArgs e)
        {
            // Reload current page with same credentials
            await Navigation.PushAsync(new EventSpaceOwnerJoinPage(_viewModel.Username, _viewModel.FormattedEmail));
        }
        ...
        {
            if (BindingContext is EventSpaceOwnerJoinViewModel vm)
